...
public class CaveManMovement : MonoBehaviour, IFinishable
{
    [Header("Movement")]
    ...
    private CaveManHealth caveManHealth;
    ...
    public GameObject boomerangHandObject;

    private bool boomerangInFlight;
    void Awake()
    {
        caveManHealth = GetComponent<CaveManHealth>();
    ...
    void HandleMovement()
    {
        ...
        if (caveManHealth.IsKnockedBack) return;
        if (isBlocking) return;
        if (GetComponent<CaveManHealth>().IsKnockedBack) return;
    ...
    public void SpawnHead()
    {
        ...
        Destroy(head, 2f); // cleanup after 5 seconds
    }
    public void ThrowBoomerang()
    {
        ...
        // Hide held boomerang
        if (boomerangHandObject != null)
            boomerangHandObject.SetActive(false);

        // Spawn using prefab's own rotation and scale
        GameObject boomerang = Instantiate(
            boomerangPrefab,
            boomerangSpawnPoint.position,
            boomerangPrefab.transform.rotation
        );
        boomerang.transform.localScale = boomerangPrefab.transform.localScale;
         
        CaveManBoomerang script = boomerang.GetComponent<CaveManBoomerang>();
        if (script != null)
            script.Init(transform, boomerangSpawnPoint);
    }
    public void OnBoomerangReturned()
    {
        boomerangInFlight = false;

        // Re-enable held boomerang
        if (boomerangHandObject != null)
            boomerangHandObject.SetActive(true);

        animator.SetTrigger("CatchBoomerang");
