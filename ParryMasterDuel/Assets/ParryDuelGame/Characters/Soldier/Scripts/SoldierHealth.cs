...
public class SoldierHealth : MonoBehaviour
{
    ...
    public void TakeDamage(float amount, Vector2 attackerPosition)
    {
        ...
        if (movement.IsInParryWindow())
        {
            ...
            KnightMovement attackerK = FindAttacker<KnightMovement>(attackerPosition);
            if (attackerK != null)
            {
        ...
        if (movement.IsBlocking())
        {
            ...
            Debug.Log($"Soldier blocked! Took reduced damage: {reducedDamage}. HP left: {currentHealth}");
        ...
        if (bloodPrefab != null && bloodSpawnPoint != null)
            Instantiate(bloodPrefab, bloodSpawnPoint.position, Quaternion.identity);
        Debug.Log($"Soldier took {amount} damage. HP left: {currentHealth}");
        if (currentHealth <= 0f)
            Die();
    }
